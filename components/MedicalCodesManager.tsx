        ...
        <div className="space-y-8 pb-20 animate-fade-in-up max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
                        ...
                        {searchResults.groups.map(group => (
                            ...
                            >
                                ...
                                <div className="bg-purple-50 dark:bg-purple-900/20 p-6 border-b border-purple-100 dark:border-purple-800/50 relative">
                                    <div className="flex justify-between items-start mb-1">
                                        <div>
                                            ...
                                            <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-tight pr-8">
                                                {group.name}
                                            </h3>
                                            {group.description && <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{group.description}</p>}
                                        </div>
                                        <div className="text-right">
                                            <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{group.codeIds.length}</div>
                                            <div className="text-[9px] font-bold text-slate-400 uppercase">Tests</div>
                                        </div>
                                    </div>
                                ...
                                <div className="p-4 flex-1 bg-white dark:bg-slate-900 overflow-y-auto max-h-[300px] custom-scrollbar space-y-3">
                                        ...
                                        return (
                                            <div key={codeId} className="border-b border-gray-100 dark:border-gray-800 pb-2 last:border-0 last:pb-0">
                                                <div className="flex justify-between items-start">
                                                    <span className="text-sm font-bold text-slate-800 dark:text-slate-200 leading-snug">{code.name}</span>
                                                </div>
                                                <div className="flex flex-wrap gap-2 mt-2">
                                                    {code.cptCode && (
                                                        <div className="group/chip flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-[10px] cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors" onClick={(e) => { e.stopPropagation(); handleCopy(code.cptCode) }}>
                                                            <span className="text-slate-400 font-bold">CPT:</span>
                                                            <span className="font-mono font-bold text-slate-700 dark:text-slate-300">{code.cptCode}</span>
                                                            <i className="fa-regular fa-copy ml-1 opacity-0 group-hover/chip:opacity-100 text-slate-400"></i>
                                                        </div>
                                                    )}
                                                    {code.labCode && (
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    {canEdit && (
                                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => openCodeModal(code)} className="text-slate-400 hover:text-medical-500"><i className="fa-solid fa-pen"></i></button>
                                            <button onClick={() => { if (window.confirm('Delete code?')) onDeleteCode(code.id) }} className="text-slate-400 hover:text-red-500"><i className="fa-solid fa-trash"></i></button>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="space-y-3 mt-auto">
                                    ...
                                    {code.labCode && (
                                        <div className="flex justify-between items-center p-3 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30">
                                            <span className="text-xs font-bold text-blue-400 uppercase tracking-wider">Lab Code</span>
                                            <div className="flex items-center gap-2">
                                                <span className="font-mono font-bold tabular-nums text-lg text-blue-700 dark:text-blue-300">
                                                    {code.labCode}
                                                </span>
                                                <button
                                    ...
                                    {code.adminCode && (
                                        <div className="flex justify-between items-center p-3 rounded-xl bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-900/30">
                                            <span className="text-xs font-bold text-purple-400 uppercase tracking-wider">Admin Code</span>
                                            <div className="flex items-center gap-2">
                                                <span className="font-mono font-bold tabular-nums text-lg text-purple-700 dark:text-purple-300">
                                                    {code.adminCode}
                                                </span>
                                                <button
                        ...
                        {searchResults.codes.length === 0 && searchResults.groups.length === 0 && (
                            <div className="col-span-full py-24 text-center">
                                <div className="inline-block p-8 rounded-full bg-slate-50 dark:bg-slate-800 mb-6 animate-bounce-slow shadow-sm">
                                    <i className="fa-solid fa-magnifying-glass text-5xl text-slate-300 dark:text-slate-600"></i>
                                </div>
                                <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">No codes or panels found</h3>
                ...
                <div className="space-y-6">
                    {canEdit && (
                        <button
                            ...
                            className="w-full h-16 bg-gradient-to-r from-medical-600 to-emerald-600 text-white rounded-2xl font-bold shadow-lg flex items-center justify-center gap-3 transition-all hover:scale-[1.01]"
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {groups.length === 0 ? (
                            <div className="col-span-full text-center py-20 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                                <i className="fa-solid fa-boxes-stacked text-4xl text-slate-300 mb-4"></i>
                                <p className="font-bold text-slate-500">No Exam Panels Created Yet</p>
                            </div>
                        ) : (
                            groups.map(group => (
                                <div
                                    key={group.id}
                                    onClick={() => { setViewingGroup(group); setViewSearchTerm(''); }}
                                    className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 cursor-pointer hover:shadow-md transition-all duration-300 group"
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{group.name}</h3>
                                            {group.description && <p className="text-slate-500 dark:text-slate-400 mt-1">{group.description}</p>}
                                        </div>
                                        {canEdit && (
                                            <div className="flex gap-2">
                                                <button onClick={(e) => { e.stopPropagation(); openGroupModal(group); }} className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-medical-500 hover:text-white transition-colors text-slate-500"><i className="fa-solid fa-pen"></i></button>
                                                <button onClick={(e) => { e.stopPropagation(); if (window.confirm('Delete group?')) onDeleteGroup(group.id); }} className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-red-500 hover:text-white transition-colors text-slate-500"><i className="fa-solid fa-trash"></i></button>
                                            </div>
                                        )}
                                    </div>

                                    <div className="grid grid-cols-1 gap-2 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl max-h-[200px] overflow-y-auto custom-scrollbar">
                                        {group.codeIds.map(codeId => {
                                            const code = codes.find(c => c.id === codeId);
                                            if (!code) return null;
                                            return (
                                                <div key={codeId} className="flex justify-between items-center text-sm">
                                                    <span className="font-bold text-slate-700 dark:text-slate-300 truncate pr-2 w-2/3">{code.name}</span>
                                                    <span className="font-mono text-xs text-slate-400">{code.cptCode}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
            ...
            {isCodeModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm" onClick={() => setIsCodeModalOpen(false)}>
                    <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg p-6 shadow-2xl animate-fade-in-up border border-slate-100 dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">{editingCode.id ? 'Edit Code' : 'Add New Code'}</h3>
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full h-12 px-4 rounded-xl bg-slate-50 dark:bg-slate-800 border-none font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500/20"
                                    placeholder="e.g. Rapid Strep"
                                />
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">CPT Code</label>
                                <input
                                    value={editingCode.cptCode || ''}
                                    onChange={e => setEditingCode({ ...editingCode, cptCode: e.target.value })}
                                    className="w-full h-12 px-4 rounded-xl bg-slate-50 dark:bg-slate-800 border-none font-bold font-mono text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500/20"
                            ...
                            <div className="flex gap-4">
                                ...
                                <div className="flex-1">
                                    <label className="text-xs font-bold text-slate-500 uppercase">Admin Code</label>
                                    <input
                        ...
                        <div className="flex gap-3 mt-8">
                            <button disabled={isLoadingCodes} onClick={() => setIsCodeModalOpen(false)} className="flex-1 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 font-bold text-slate-600 dark:text-slate-300">Cancel</button>
            ...
            {isGroupModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm" onClick={() => setIsGroupModalOpen(false)}>
                    <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-4xl p-8 shadow-2xl animate-fade-in-up border border-slate-100 dark:border-slate-800 flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                        <h3 className="text-3xl font-bold text-slate-900 dark:text-white mb-6">{editingGroup.id ? 'Edit Panel' : 'Create New Panel'}</h3>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Panel Name</label>
                                <input
                                    value={editingGroup.name || ''}
                                    onChange={e => setEditingGroup({ ...editingGroup, name: e.target.value })}
                                    className="w-full h-14 px-5 rounded-2xl bg-slate-50 dark:bg-slate-800 border-none font-bold text-xl text-slate-900 dark:text-white focus:ring-4 focus:ring-purple-500/10 placeholder-slate-400"
                                    placeholder="e.g. Routine Labs"
                                />
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Description (Optional)</label>
                                <input
                                    value={editingGroup.description || ''}
                                    onChange={e => setEditingGroup({ ...editingGroup, description: e.target.value })}
                                    className="w-full h-14 px-5 rounded-2xl bg-slate-50 dark:bg-slate-800 border-none font-bold text-lg text-slate-900 dark:text-white focus:ring-4 focus:ring-purple-500/10 placeholder-slate-400"
                                    placeholder="Short description"
                                />
                            </div>
                        </div>

                        <div className="flex-1 overflow-hidden flex flex-col min-h-0 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800">
                            {/* Search Area */}
                            <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-12 pl-12 pr-4 bg-slate-100 dark:bg-slate-800 rounded-xl text-base font-bold outline-none text-slate-900 dark:text-white placeholder-slate-500 focus:ring-2 focus:ring-purple-500/50 transition-all"
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                                <div className="flex justify-between items-center mb-2 px-1">
                                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Selected Codes ({editingGroup.codeIds?.length || 0})</span>
                                    {editingGroup.codeIds?.length === 0 && <span className="text-xs text-slate-400 italic">List is empty</span>}
                                </div>
                                ...
                                {editingGroup.codeIds?.map(id => {
                                    ...
                                    if (!code) return null;
                                    return (
                                        <div key={id} className="flex flex-col md:flex-row md:items-center justify-between p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 hover:border-purple-300 transition-colors gap-4 animate-fade-in-up">
                                            <div className="flex-1">
                        ...
                        <div className="flex gap-4 mt-8 pt-4 border-t border-gray-100 dark:border-gray-800">
                            <button disabled={isLoadingCodes} onClick={() => setIsGroupModalOpen(false)} className="flex-1 h-14 rounded-2xl border-2 border-slate-100 dark:border-slate-800 font-bold text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Cancel</button>
            ...
            {viewingGroup && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm" onClick={() => setViewingGroup(null)}>
                    <div
                        className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-3xl shadow-2xl animate-fade-in-up border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col max-h-[85vh] relative"
                    ...
                    >
                        ...
                        {/* Controls Bar */}
                        <div className="px-8 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between items-center gap-4 flex-shrink-0">
                            <div className="relative w-full md:w-auto flex-1">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-purple-500/20 text-slate-900 dark:text-white font-bold text-sm"
                                />
                            </div>
                        ...
                        {/* List */}
                        <div className="p-6 overflow-y-auto custom-scrollbar bg-white dark:bg-slate-900 space-y-3 flex-1 min-h-0">
                            {filteredViewCodes.length === 0 ? (
                                <div className="text-center py-10 text-slate-400">No codes found matching "{viewSearchTerm}"</div>
                            ) : (
                                filteredViewCodes.map((code, idx) => (
                                    <div key={code.id} className={`p-5 rounded-2xl border border-slate-100 dark:border-slate-700 transition-colors group ${idx % 2 === 0 ? 'bg-slate-50/50 dark:bg-slate-800/30' : 'bg-white dark:bg-slate-800/10'} hover:border-purple-300 dark:hover:border-purple-700`}>
                                        <div className="flex justify-between items-start mb-3">
                                            <h4 className="text-xl font-bold text-slate-900 dark:text-white">{code.name}</h4>
                                        </div>

                                        <div className="flex flex-wrap gap-3">
                                            {code.cptCode && (
                                                <div className="flex items-center gap-3 bg-white dark:bg-slate-800 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors" onClick={() => handleCopy(code.cptCode)}>
                                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">CPT</span>
                                                    <span className="font-mono font-bold text-slate-900 dark:text-white text-lg">{code.cptCode}</span>
                                                    <i className="fa-regular fa-copy text-slate-300 text-xs ml-1"></i>
                                                </div>
                                            )}
                                            {code.labCode && (
                        ...
                        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-end flex-shrink-0">
                            <button
                                onClick={() => setViewingGroup(null)}
                                className="px-8 py-3 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-sm uppercase tracking-wide"
                            >
                                Close Window
                            </button>
                        </div>
